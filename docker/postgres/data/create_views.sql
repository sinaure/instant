CREATE TABLE mutations_by_year ( aneemut integer,  transactions bigint,  totval float, mean_val float, CONSTRAINT mutations_by_year_pk PRIMARY KEY (aneemut));

CREATE MATERIALIZED VIEW mutations_loc_vw AS SELECT 
t1.*,
...
;
CREATE VIEW mutations_loc_view  AS SELECT mv.* from mutations_loc_vw mv;

CREATE MATERIALIZED VIEW mutations_parcel_vw AS SELECT 
t1.*,
substr(t1.l_codinsee[1],3,3)::integer as code_com,
